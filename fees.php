						...
						<table class="table table-condensed table-bordered table-hover">
							<thead>
								<tr>
									...
									<th class="">Balance</th>
									<th class="">Cuotas</th>
									<th class="">Cuotas Restantes</th>
									<th class="">Valor Cuotas</th>
									<th class="">Concepto</th>
									<th class="">Inicio de clases</th>
							...
							<tbody>
								...
								while ($row = $fees->fetch_assoc()) :
									...
									$cuota = $conn->query("SELECT sum(cuotas_pagadas) as cuota FROM payments where ef_id=" . $row['id']);
									$cuota = $cuota->num_rows > 0 ? $cuota->fetch_array()['cuota'] : '';
									$total_cuota = $row['cuotas'] - $cuota;
									$sumcuota = $row['total_fee'] / $row['cuotas'];
								...
								?>
									<tr>
										...
										<td class="text-right">
											<p><?php echo number_format($row['cuotas'], 2) ?></p>
										</td>
										<td class="text-right">
											<p><?php echo number_format($total_cuota, 2) ?></p>
										</td>
										<td class="text-right">
											<p><?php echo number_format($sumcuota, 2) ?></p>
										</td>
										<td class="text-right">
											<p> <?php echo $row['concepto'] ?></p>
										</td>
										<td class="text-right">
											<p><?php echo date("M d,Y", strtotime($row['inicioclases'])) ?></p>
										</td>
